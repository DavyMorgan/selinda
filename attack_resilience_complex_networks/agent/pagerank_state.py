...
def pagerank_state(network: nx.Graph, obs: dict[str, np.ndarray], oneshot: bool=False) -> int:

    node_features, valid_actions, indices = unpack_obs(obs)
    if len(valid_actions):
        pr = nx.pagerank(network)
        pr_score = np.array([pr[i] for i in range(len(pr))])
        stable_state = node_features[:, -4][valid_actions]
        sr_score = pr_score * stable_state
        if not oneshot:
