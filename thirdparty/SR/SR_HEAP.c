...
//get CI for node i
dbl_t get_CI(int_t i, varNode *Node, int_t N, int_t **Graph, int L, int_t *queue, int_t *check, int_t *lenght) {
    int_t *r, *w, temp, delta, cnt, k, neigh, index, deg;
    dbl_t CI;
    int s;

    // if (Node[i].deg == 0 || Node[i].deg == 1) { return 0; }
    if (Node[i].deg <= 1) { return 0; }
    else {
        queue[0] = i;
        check[i] = ON;
        r = queue;
        ...
        for (k = index; k < (index + lenght[L]); k++) {
            d_bar += (double) Node[queue[k]].deg;
        }
        d_bar = d_bar / (double) lenght[L];
        dbl_t d = (double) Node[i].deg;
        CI = d*d / d_bar;
...
//get influencers
int_t *get_influencers(varNode *Node, int_t N, int_t **Graph, int L, int_t STOP_CONDITION, bool oneshot, bool use_reinsert) { // int ntwk) {
    int_t i, j, cnt, toBeRemoved, currentNode, pos_currentNode, NumNodesToUpdate, NumLink;
    dbl_t CI_ave;
...
//    sprintf(fname_infl, "INFLUENCERS_%s_lvl_%d.txt", network, L);
    ...
    //GET INFLUENCERS
    listInfluencers = get_influencers(Node, N, Graph, L, stop_condition, oneshot, use_reinsert); //, ntwk);
